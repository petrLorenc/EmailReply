...
def get_pos(sentence, stopwords, to_text):
  text_pos = nltk.pos_tag(nltk.word_tokenize(get_lemmatized_words(sentence, stopwords)))
  if to_text:
    return " ".join([str(word) + " " + str(nltk.map_tag('en-ptb', 'universal', tag)) for word, tag in text_pos])
  else:
...
def load_file_to_process(path_to_file, path_to_stopwords, to_text):
    ...
    for sentence in input_x:
      yield get_pos(sentence, stopwords, to_text)
