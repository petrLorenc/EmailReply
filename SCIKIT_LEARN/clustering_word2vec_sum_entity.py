...
SIZE_WORD2VEC_VECTOR = 300

# using coordinate-wise minimum - minimum from all first elements in vector and so on

TEXT_DATA = "./data/train_5500.label.replace_entity"
TAG_DATA = "./data/train_5500.label.tag"
MODEL_DIR = "./word2vec_data/GoogleNews-vectors-negative300.bin"
...
def sentence_2_vec(model, sentence):
    ...
    for word in sentence.split(): 
        if word in model.vocab:
            try:
                vectors.append(model[word.strip().lower()])
            except:
                pass
    #print(vectors)
    vectors = np.matrix(np.array(vectors).reshape(-1, SIZE_WORD2VEC_VECTOR))
    #print(vectors)
    sumVector = []
    for i in range(SIZE_WORD2VEC_VECTOR):
        sumVector.append(np.sum(vectors[:,i]))
    return sumVector
...
def main():
    ...
    #post_X_train = post_X_train.reshape(-1, SIZE_WORD2VEC_VECTOR)
