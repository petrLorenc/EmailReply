...
x = Conv1D(128, 4, strides=1 , activation='relu')(embedded_sequences) 
x = MaxPooling1D(2)(x) # 2*39 -> 39
...
name_model = "model_CNN_300_10_reg_bigger_window"

if not os.path.exists(SAVE_DIR):
    os.makedirs(SAVE_DIR)
# serialize model to JSON
model_json = model.to_json()
with open(os.path.join(SAVE_DIR, name_model+ ".json"), "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights(os.path.join(SAVE_DIR,name_model + ".h5"))
